...
python3.12 -m nuitka --macos-create-app-bundle --enable-plugin=tk-inter --include-data-files=PipHandler.py=PipHandler.py --include-data-files=Version.json=Version.json --macos-target-arch="x86_64" --output-dir="./Apps" --include-package-data=pyobjc --include-package-data=Cocoa --include-package-data=Quartz --macos-app-icon=./BootstrapImages/AppIcon.icns --target="OrangeBlox" "./Apps/Scripts/OrangeBlox.py"
