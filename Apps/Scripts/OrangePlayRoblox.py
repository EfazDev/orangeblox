...
if __name__ == "__main__":
    ...
    logging_config = setLoggingHandler("OrangePlayRoblox")
    ...
    if main_os == "Darwin":
        ...
        if os.path.exists(os.path.join(app_path, "/Contents/MacOS/OrangeBlox.app/")):
            ...
            with open(url_scheme_path, "w", encoding="utf-8") as f: f.write("orangeblox://continue")
        ...
        if os.path.exists(os.path.join(app_path, "OrangeBlox.exe")):
            ...
            filtered_args = ""
            if len(args) > 1: filtered_args = "obx-launch-player " + " ".join(args)
            write_arg = filtered_args if filtered_args != "" else "orangeblox://continue"
