...
if [ "$ma_os" = "Darwin" ]; then
    url="https://www.python.org/ftp/python/3.14.3/python-3.14.3-macos11.pkg"
    ...
    if [ "$arch" = "x86_64" ]; then
        if [ "$PROCESSOR_ARCHITEW6432" == "ARM64" ]; then
            url="https://www.python.org/ftp/python/3.14.3/python-3.14.3-arm64.exe"
        else
            url="https://www.python.org/ftp/python/3.14.3/python-3.14.3-amd64.exe"
        fi
    else
        url="https://www.python.org/ftp/python/3.14.3/python-3.14.3.exe"
    fi
