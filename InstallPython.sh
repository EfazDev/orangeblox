...
if [ "$ma_os" = "Darwin" ]; then
    url="https://www.python.org/ftp/python/3.14.0/python-3.14.0-macos11.pkg"
    ...
    if [ "$arch" = "x86_64" ]; then
        if [ "$PROCESSOR_ARCHITEW6432" == "ARM64" ]; then
            url="https://www.python.org/ftp/python/3.14.0/python-3.14.0-arm64.exe"
        else
            url="https://www.python.org/ftp/python/3.14.0/python-3.14.0-amd64.exe"
        fi
    else
        url="https://www.python.org/ftp/python/3.14.0/python-3.14.0.exe"
    fi
