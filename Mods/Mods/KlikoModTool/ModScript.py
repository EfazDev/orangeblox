...
latest = OrangeAPI.getLatestRobloxVersion(binary_type="WindowsStudio64")
if latest:
    installed = {
...
if passed:
    ...
    if not (isinstance(cur_config.get("ReadDisclaimer2026.5"), bool)):
                ...
                for mod in provided_mods:
                    if os.path.isdir(os.path.join(mod_packs_dir, mod)) and os.path.exists(os.path.join(mod_packs_dir, mod, "GenerativeMod.json")):
                        try:
                            ...
                            if not isinstance(configurations[mod], dict): configurations.pop(mod)
                        except: pass
                indexed = {}
    ...
    if not (isinstance(cur_config.get("EnabledRobloxStudio"), bool)):
    ...
    if cur_config.get("EnabledRobloxStudio") == False and studio == True:
        ...
        mod_style_file = None
    if mod_style_file:
        if isinstance(mod_style_file, str) and os.path.isfile(mod_style_file):
            try:
                with open(mod_style_file, "r", encoding="utf-8") as f: mod_style_json = json.load(f)
                if mod_style_json.get("name") and (mod_style_json.get("colors") or mod_style_json.get("advanced")) and isinstance(mod_style_json.get("angle"), int):
                    if reinstall_mode == True or not (cur_config.get(f"LastUpdatedRoblox{'Studio' if studio else ''}") == installed["version"] and cur_config.get(f"KlikoModVersion{'Studio' if studio else ''}") == current_version):
                        ...
                        if mod_style_json.get("advanced"):
                            ...
                            def generate_f(key: str, file_path: str):
                                safe_key = key.replace("/", "_")
                                return os.path.join(resources_folder, f"{safe_key}{file_extension(file_path)}")
                            ...
                            for i, v in mod_style_json["advanced"].items():
                                if isinstance(v, str) and os.path.exists(convert_relative(v)): 
                                    out_path = generate_f(i, v)
                                    if not os.path.exists(os.path.dirname(out_path)): os.makedirs(os.path.dirname(out_path), mode=511)
                                    shutil.copy(convert_relative(v), out_path)
                                    mod_style_json["advanced"][i] = out_path
                                elif isinstance(v, dict) and "image" in v and isinstance(v["image"], str):
                                    rel_path = convert_relative(v["image"])
                                    if os.path.exists(rel_path):
                                        out_path = generate_f(i, v["image"])
                                        if not os.path.exists(os.path.dirname(out_path)): os.makedirs(os.path.dirname(out_path), mode=511)
                                        shutil.copy(rel_path, out_path)
                                        mod_style_json["advanced"][i]["image"] = out_path
