...
def run(versions: str, name: str, colors: list[str], angle: int, studio: bool=False, user_selected_files: list[dict[str, Path | list[str]]] | None = None) -> None:
    try:
        ...
        try:
            ...
            else: version: str = versions['version']; channel = "LIVE"; macos = platform.system() == "Darwin" and False
            ...
            else: version: str = versions['version']; macos = platform.system() == "Darwin" and False
            channel = "LIVE"
        ...
        with tempfile.TemporaryDirectory(prefix="KlikoModGenerator") as tmpdir:
            temporary_directory: Path = Path(tmpdir)
            ...
            progress_bar.submit("[MOD_GEN] Locating Assets..", 50)
            imageset_paths, imagesetdata_paths, builderfont_paths = img_sets.locate_all_assets(temporary_directory / studio_version)
            ...
            img_sets.generate_colored_fonts(temp_target, builderfont_paths, colors, angle)
            ...
            if os.path.exists(os.path.join(tmpdir, name)):
                shutil.move(os.path.join(tmpdir, name), os.path.join(cur_path, "result", name))
