...
def detect_modded_icons(modded_imagesets: Path, unmodded_imagesets: Path, icon_map: dict[str, dict[str, dict[str, str | int]]]) -> dict[str, list[str]]:
    ...
    for size, icons in icon_map.items():
        for icon, data in icons.items():
            ...
            if not modded_image_set_path.is_file(): continue
            if not unmodded_image_set_path.is_file(): raise ImageSetsNotFoundError(f"Failed to find unmodded ImageSet: {image_set}.png")
            with Image.open(modded_image_set_path, formats=("PNG",)) as file: modded_icon: Image.Image = file.convert("RGBA").crop((x, y, x+w, y+h))
            with Image.open(unmodded_image_set_path, formats=("PNG",)) as file: unmodded_icon: Image.Image = file.convert("RGBA").crop((x, y, x+w, y+h))
            if not is_modded_image(modded_icon, unmodded_icon): continue
            if size not in modded_icons: modded_icons[size] = []
...
def is_modded_image(image1: Image.Image, image2: Image.Image) -> bool:
    if image1.size != image2.size: return True
    # Ensure alpha channel
    if image1.mode != "RGBA": image1 = image1.convert("RGBA")
    if image2.mode != "RGBA": image2 = image2.convert("RGBA")
    pixels1 = image1.load()
    pixels2 = image2.load()
