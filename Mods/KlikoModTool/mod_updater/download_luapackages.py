...
def download_luapackages(version: str, channel: str, output_directory: str | Path, studio: bool) -> None:
    ...
    if studio == True:
        ...
        progress_bar.submit("[MOD_UPDATE] Downloading Extra Content Textures..", 30)
        download(Api.Roblox.Deployment.download(version, "extracontent-textures.zip", channel, False), output_directory / "download" / f"{version}-extracontent-textures.zip")
        ...
        progress_bar.submit("[MOD_UPDATE] Downloading Lua Packages..", 40)
        download(Api.Roblox.Deployment.download(version, "extracontent-luapackages.zip", channel, False), output_directory / "download" / f"{version}-extracontent-luapackages.zip")
        ...
        progress_bar.submit("[MOD_UPDATE] Downloading Textures..", 45)
        download(Api.Roblox.Deployment.download(version, "content-textures2.zip", channel, False), output_directory / "download" / f"{version}-content-textures2.zip")
        extract(os.path.join(output_directory, "download", f"{version}-content-textures2.zip"), output_directory / version / "content" / "textures")
        download(Api.Roblox.Deployment.download(version, "content-textures3.zip", channel, False), output_directory / "download" / f"{version}-content-textures3.zip")
