...
from mod_updater.locate_imagesets import locate_imagesets
from mod_updater.locate_imagesetdata_file import locate_imagesetdata_file
from mod_updater.get_icon_map import get_icon_map
...
def update_mods(check: dict[str, list[Path]], versions: str) -> None:
    ...
    try:
        ...
        if os.path.exists(os.path.join(current_path_location, "test")):
...
def hash_specific_worker_thread(deploy_history: DeployHistory, exception_queue: Queue, hash: str, mods: list[Path], output_directory: Path) -> None:
    global channel
    if hash == deploy_history.get_hash(deploy_history.LatestVersion.studio) or not mods:
        return

    progress_bar.submit("[MOD_UPDATE] Fetching Studio Version!", 5)
    ...
    try:
        ...
        for mod in mods:
            ...
            if different_versions.get(data["clientVersionUpload"]):
                different_versions[data["clientVersionUpload"]][1].append(mod)
            else:
                different_versions[data["clientVersionUpload"]] = [data.get("channel", "LIVE"), [mod]]
        ...
        for i, v in different_versions.items():
            ...
            mod_imageset_path: Path = locate_imagesets(temporary_directory / i)
            ...
            mod_imagesetdata_path: Path = locate_imagesetdata_file(temporary_directory / i)
