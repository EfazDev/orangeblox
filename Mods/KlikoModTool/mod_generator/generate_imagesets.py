...
from mod_generator.modules.request import Response, Api
from mod_generator.get_mask import get_mask, clear_cache
...
def generate_imagesets(
    ...
    for _, icons in icon_map.items():
        for icon_name, data in icons.items():
            ...
            if "AE/LuaApp" in str(image_set_path.absolute()): image_set_path = Path(str(image_set_path).replace("AE/LuaApp", "AE"))
            if "InGameMenu/LuaApp" in str(image_set_path.absolute()): image_set_path = Path(str(image_set_path).replace("InGameMenu/LuaApp", "InGameMenu"))
            if "LuaApp/LuaApp" in str(image_set_path.absolute()): image_set_path = Path(str(image_set_path).replace("LuaApp/LuaApp", "LuaApp"))
    ...
    for image_set, image_set_data in formatted_icon_map.items():
        ...
        with Image.open(path, formats=("PNG",)) as image:
            ...
            for box in image_set_data["icons"]:
                ...
                modded_icon = get_mask(colors, angle, icon.size)
